...
const getMediaInfo = async (): Promise<void> => {
  const mediainfo = await window.electron.ipcRenderer.invoke('electron:getMediaInfo')
  if (!isUndefined(mediainfo)) {
    Modal.info({
      title: '媒体信息',
      content: JSON.stringify(mediainfo)
    })
  } else {
    Modal.error({
      title: 'error'
...
<template>
  <Versions></Versions>
  <a-button type="primary" @click="getMediaInfo()">Primary Button</a-button>
